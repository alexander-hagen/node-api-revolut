...
RevolutPrivate.prototype.query = async function(options) {
  ...
  const uri=parts[0];
  const query=parts[1];
  const payload=stamp+options.method.toUpperCase()+uri+(query!==undefined?query:"")+(options.data!==undefined?JSON.stringify(options.data):"");
  ...
  try {
    ...
    if(!err.hasOwnProperty("response")) { Object.assign(response,{ status: "503", error: err.code }); }
      ...
      Object.assign(response,{ status: err.response.status, error: err.response.statusText, retry_after: (retry==undefined?0:retry) });
...
RevolutPrivate.prototype.getAllBalances = async function() { // https://developer.revolut.com/docs/x-api/get-all-currencies
  ...
  return await new Promise(async (resolve, reject) => {
    ...
    if (result?.error_id || (result?.status !== undefined && result.status !== 200)) { reject(result); } else { resolve(result); };
  });
};
...
RevolutPrivate.prototype.cancelOrders = async function() { // https://developer.revolut.com/docs/x-api/cancel-all-orders
  const path="/api/1.0/orders";
  return await new Promise(async (resolve, reject) => {
...
RevolutPrivate.prototype.replaceOrder = async function(id,options) { // https://developer.revolut.com/docs/x-api/replace-order
  const path="/api/1.0/orders/"+id;
  return await new Promise(async (resolve, reject) => {
...
RevolutPrivate.prototype.getOrderbook = async function(symbol,limit=20) { // https://developer.revolut.com/docs/x-api/get-order-book
